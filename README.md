...
The break page stays reloadable from its clean loopback URL while the intervention is unresolved. Its session slides: every authenticated request, including the page's own low-frequency loopback heartbeat, extends it by thirty minutes, so an open page stays usable while you think about it. Thirty minutes with no activity at all expires it. Choosing Take a break, Snooze, or Keep going — or stopping the monitor — expires it immediately, and the URL then reports that no intervention is active. If a session does expire while the intervention is still unresolved, the page says so plainly instead of claiming there is none, and points you at `open-clowk status`. Nothing is stored in the browser beyond that short-lived cookie.
