...
const NOTICE_STYLE = "body{margin:0;min-height:100vh;display:grid;place-items:center;background:#12100e;color:#fff5e6;font:16px/1.6 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;text-align:center}p{max-width:34rem;padding:0 24px;color:#cdbba4}strong{color:#ef6129}code{color:#ffbe87}";

function noticePage(title, paragraphs) {
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Open Clowk — ${title}</title>
<style>${NOTICE_STYLE}</style>
</head><body><main>${paragraphs.map((text) => `<p>${text}</p>`).join('\n')}</main></body></html>
`;
}

const RESOLVED_PAGE = noticePage('no active intervention', [
  '<strong>Open Clowk</strong> has no active intervention right now.',
  'This local page opens itself when your threshold is reached. Nothing was closed, killed, or blocked.',
]);

const EXPIRED_PAGE = noticePage('session expired', [
  'This page&#39;s session expired, but <strong>Open Clowk</strong> is still waiting for your choice.',
  'Close the watched app or restart Open Clowk to reset.',
  'Run <code>open-clowk status</code> to see the current state, or <code>open-clowk stop</code> then <code>open-clowk start</code> to begin a fresh cycle. Nothing was closed, killed, or blocked.',
]);
...
async function runMonitor({
  ...
  const handle = (request, response) => {
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      ...
      if (!open) {
        sendHtml(response, 403, interventionActive() ? EXPIRED_PAGE : RESOLVED_PAGE, {
