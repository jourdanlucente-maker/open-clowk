...
const { monitorStatus, portRefused, stopMonitorRequest } = require('./control');
const {
  acquireMonitorLock,
  readConfig,
...
async function startMonitor({ env = process.env, spawnFn = spawn, waitFn = wait } = {}) {
  ...
  if (!acquireMonitorLock(token, { env })) {
    ...
    if (!acquired) {
      throw new Error('Another live process owns the per-user start lock. Run open-clowk stop, then start again.');
    }
  }
...
async function stopMonitor({ env = process.env, waitFn = wait, probeFn = portRefused } = {}) {
  let current = await monitorStatus({ env });
  if (!current.running) {
    await waitFn(250);
    current = await monitorStatus({ env });
  }
  if (!current.running) {
    const port = current.runtime?.port;
    if (port && await probeFn(port)) releaseMonitorState({ env });
    else reclaimAbandonedState({ env });
    return false;
  }
