...
async function listProcesses({
  ...
  const output = await execFileFn(command, args, {
    encoding: 'utf8',
    windowsHide: true,
    timeout: 5000,
...
async function openBrowser(url, {
  ...
  await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => settle(), exitGraceMs);
    child.on('error', (error) => settle(new Error(`Could not launch ${command}: ${error.message}`)));
    child.on('spawn', () => child.unref());
    child.on('exit', (code, signal) => {
      if (code === 0) settle();
      else settle(new Error(`${command} exited with ${signal ? `signal ${signal}` : `code ${code}`} without opening the break page.`));
    });
  });
}
