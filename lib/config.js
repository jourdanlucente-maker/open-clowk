...
const BOOT_CLOCK_SKEW_MS = 300000;
...
function lockRecord(token, options) {
  return `${JSON.stringify({
    version: 1,
    token,
    pid: options.pid || process.pid,
    startedAt: (options.nowFn || Date.now)(),
  })}\n`;
}
...
function acquireMonitorLock(token, options = {}) {
  ...
  try {
    ...
    fs.writeFileSync(descriptor, lockRecord(token, options));
    return true;
  } catch (error) {
...
function claimMonitorLock(token, options = {}) {
  ...
  writeFileAtomic(lockPath(options), lockRecord(token, options));
  return true;
}
...
function readMonitorLockRecord(options = {}) {
  ...
  return { token: raw, pid: null, startedAt: null };
}
...
function bootTime({ uptimeFn = os.uptime, nowFn = Date.now } = {}) {
  return nowFn() - uptimeFn() * 1000;
}

function ownerAlive(record, options = {}) {
  if (!record || !Number.isInteger(record.pid) || record.pid <= 0) return false;
  if (Number.isInteger(record.startedAt) && record.startedAt + BOOT_CLOCK_SKEW_MS < bootTime(options)) return false;
  return processAlive(record.pid, options);
}

function reclaimAbandonedState(options = {}) {
  let reclaimed = false;
  const lock = readMonitorLockRecord(options);
  if (lock && !ownerAlive(lock, options)) {
    reclaimed = removeMonitorLock(lock.token, options) || reclaimed;
  }
  const runtime = readRuntime(options);
  if (runtime && !ownerAlive(runtime, options)) {
    reclaimed = removeRuntime(runtime.token, options) || reclaimed;
  }
  return reclaimed;
}

function releaseMonitorState(options = {}) {
  const lock = readMonitorLockRecord(options);
  const runtime = readRuntime(options);
  let released = false;
  if (lock) released = removeMonitorLock(lock.token, options) || released;
  if (runtime) released = removeRuntime(runtime.token, options) || released;
  return released;
}

function removeMonitorLock(expectedToken, options = {}) {
  const current = readMonitorLock(options);
  if (current && expectedToken && current !== expectedToken) return false;
  try {
    fs.unlinkSync(lockPath(options));
  } catch (error) {
    if (error.code === 'ENOENT') return false;
    throw error;
  }
