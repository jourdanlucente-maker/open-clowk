...
The intended boundary is process names only, bounded local configuration/timing state, packaged local assets, and an authenticated loopback action channel. The browser reaches that channel through a single-use bootstrap credential exchanged for a short-lived `HttpOnly`, `SameSite=Strict` session cookie. Because loopback cookies are not port-scoped, cookie-authenticated actions additionally require a same-origin request, so a page served by another `127.0.0.1` port cannot spend the session. Configuration and monitor control require the file-protected per-run token instead.
